...
namespace TdrExport.TDR2000.Formats
{
    public class HIE
    {
        ...
        {
            return Load(File.ReadAllBytes(path), Path.GetFileName(path), Path.GetDirectoryName(path));
        }

        public static HIE Load(byte[] data, string name, string baseDir = null, PakManager pak = null)
        {
            ...
            // Try to find the .h file for names
            string hName = Path.GetFileNameWithoutExtension(name) + ".h";
            byte[] hData = null;

            // 1. Try PakManager
            if (pak != null) hData = pak.LoadFile(hName);
            // 2. Try disk
            if (hData == null && baseDir != null)
            {
                string hPath = Path.Combine(baseDir, hName);
                if (File.Exists(hPath)) hData = File.ReadAllBytes(hPath);
            }

            if (hData != null)
            {
                // Note: Need to update H.Load to accept bytes too if we want full PAK support
                // For now, let's assume names are optional or H is on disk
                // Actually, let's just make a quick hack or update H.cs too
                try {
                    using var ms = new MemoryStream(hData);
                    using var sr = new StreamReader(ms);
                    h = H.LoadFromStream(sr);
                } catch {}
            }
    ...
    public class TDRMaterial
    {
        ...
        public TDRMaterial(string line)
        {
            ...
            V = new Vector4(parts[0].ToSingle(), parts[1].ToSingle(), parts[2].ToSingle(), parts[3].ToSingle());
            I = int.Parse(parts[4]);
        }
    }
}
